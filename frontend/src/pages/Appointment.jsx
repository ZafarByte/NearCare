  ...
  const bookAppointment = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        ...
        {
          headers: {
            token: token, // If your middleware expects `token`
            // Authorization: `Bearer ${token}` ← use this if your middleware expects `Authorization`
          },
        }
      );
...
return (
  <div className='px-4 md:px-0'>
    ...
    <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray'>
      ...
      <button onClick={bookAppointment} className='bg-[#0f172a] text-white text-sm font-light px-10 md:px-14 py-3 rounded-full my-4 md:my-6 w-full sm:w-auto'>Book an Appointment</button>
    </div>
